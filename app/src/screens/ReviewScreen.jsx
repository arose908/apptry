import { review } from '../data/mock.js'
    ...
    >
      ...
      <div style={{ background: 'var(--ink)', color: 'var(--paper)', borderRadius: 12, padding: 16, display: 'flex', flexDirection: 'column', gap: 8 }}>
        ...
        <div style={{ display: 'flex', gap: 8, marginTop: 4 }}>
          <button className="btn-primary" style={{ flex: 1, fontSize: 15, padding: 12, borderRadius: 9 }}>
            Do it
          </button>
          <button className="btn-dark">Leave it</button>
        </div>
      </div>
      ...
      <div style={{ marginTop: 'auto', display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: 16, borderTop: '1px solid var(--border)' }}>
        {settings.showDropCounts && (
          <span style={{ fontSize: 14, fontWeight: 600, color: 'var(--ink)' }}>
            {review.droppedCount} things dropped this week
          </span>
        )}
        <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--accent-link)' }}>CLEAR THEM ALL</span>
      </div>
    </div>
  )
}
